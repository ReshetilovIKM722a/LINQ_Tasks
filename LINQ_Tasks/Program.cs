...
class Program
{
    static void Main(string[] args)
    {
        ...
        var ElectronicProd = products.Where(x => x.Category == "Електроніка").ToList();

        // 2. Знайти найдорожчий товар.
        var MostExpensiveProd = products.Max(x => x.Price);

        // 3. Підрахувати загальну вартість всіх товарів.
        var SumProd = products.Sum(x => x.Price);

        // 4. Відсортувати товари за ціною у зростаючому порядку.
        var SortPrice = products.OrderBy(x => x.Price);
        ...
        var countDecorations = products.Count(x => x.Category == "Прикраси");
        var countAccessories = products.Count(x => x.Category == "Аксесуари");
        var countSport = products.Count(x => x.Category == "Спорт");
        var countConsumerElectronic = products.Count(x => x.Category == "Побутова техніка");

        // 7. Вибрати товари з назвами, які починаються на букву "С".
        var startsWithC = products.Where(x => x.Name[0] == 'С').ToList();
        ...
        var PriceMore100 = products.Count(x => x.Price > 100);
        ...
        var CategoryFood = products.Where(x => x.Category == "Їжа").ToList();
        var sortByPrice = products.OrderBy(x => x.Price);
